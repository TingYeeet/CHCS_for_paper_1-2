...
class PM25ThresholdAnalysis:
    ...
    def fit_cubic_model(self, x, y):
        ...
        x_cubic = cubic_features.fit_transform(x_sorted.reshape(-1, 1))
        cubic_reg = LinearRegression()
        cubic_reg.fit(x_cubic, y_sorted)
        
        # 建立更密集的x點用於分析
        x_dense = np.linspace(x_sorted.min(), x_sorted.max(), 1000)
        x_dense_cubic = cubic_features.transform(x_dense.reshape(-1, 1))
        y_dense_pred = cubic_reg.predict(x_dense_cubic)
        
        return x_sorted, y_sorted, x_dense, y_dense_pred, cubic_reg, cubic_features
    ...
    def run_analysis(self):
        ...
        save_path = "./6_Threshold_Analysis"
    ...
    def create_summary_table(self):
        ...
        # 儲存結果
        output_path = "./6_Threshold_Analysis"
